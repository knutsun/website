...
def confirm(request):
    ...
    if sub.conf_num == request.GET['conf_num']:
        sub.confirmed = True
        sub.save()
        return render(request, 'subscribe/confirmed.html', {'email': sub.email, 'action': 'confirmed'})
    else:
        return render(request, 'subscribe/confirmed.html', {'email': sub.email, 'action': 'denied'})
